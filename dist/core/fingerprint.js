...
import { fullMatch, isIdentifier, isSpace, lstrip, matchStart, py, splitLines, strip, sub, } from "./pyre.js";
// -- Paths ------------------------------------------------------------------------
...
const HTTP_STATUS_ANCHORED = py(String.raw `(?i)\bhttps?/\d(?:\.\d+)?\s+([1-5]\d{2})\b` +
    String.raw `|\bhttps?\b[^\d]{0,20}\b([1-5]\d{2})\b` +
    String.raw `|\b([1-5]\d{2})\s+(?:client|server)\s+error\b`);
const HTTP_STATUS_CONTEXTUAL = py(String.raw `(?i)\b(?:returned|status|responded\s+with|response)\b\s*(?:code\s*)?[:=]?\s*([1-5]\d{2})\b`);
...
function preserveHttpStatus(text) {
    let out = sub(HTTP_STATUS_ANCHORED, text, markHttpStatus);
    if (HTTP_STATUS_PRESENT.test(out))
        out = sub(HTTP_STATUS_CONTEXTUAL, out, markHttpStatus);
    return out;
}
// -- Exit codes, signals, ports ---------------------------------------------------
const EXIT_CODE_NUM = py(String.raw `(?i)\b(?:exit(?:\s*(?:code|status)|code|status)|exited\s+with\s+(?:exit\s+)?code|returned\s+a\s+non-zero\s+code)\b\s*[:=]?\s*(\d+)`);
...
const PORT_DNS_NUM = py(String.raw `(?i)\b(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+([a-z]{2,24}):(\d{1,5})\b(?![:.]\d)`);
const PORT_CONTEXT_NUM = py(String.raw `(?i)\b(?:connect(?:ing|ed)?\s+to|connection\s+to|dial(?:ing)?(?:\s+to)?` +
...
function glueGroup(m, prefix, group = 1) {
...
function glueNumber(m, prefix) {
    const number = firstGroup(m);
    const text = m.groups[0];
    const at = text.indexOf(number);
    return `${text.slice(0, at)}${prefix}${number}${text.slice(at + number.length)}`;
}
function glueDnsPort(m) {
    if (SOURCE_FILE_SUFFIXES.has(m.groups[1].toLowerCase()))
        return m.groups[0];
    return glueGroup(m, "netport", 2);
}
function glueContextPort(m) {
    const labels = m.groups[1].split(".");
    if (SOURCE_FILE_SUFFIXES.has(labels[labels.length - 1].toLowerCase()))
        return m.groups[0];
    return glueGroup(m, "netport", 2);
}
function preserveSemanticNumbers(text) {
...
// -- Tracebacks -------------------------------------------------------------------
const TOOL_LOOP_WARNING = py(String.raw `\s*\[Tool loop warning:\s*repeated_exact_failure_warning;[^\]]*\]\s*$`, "i");
const TRACEBACK_HEADER = py(String.raw `(?:(?:\+\s+)?Exception Group )?Traceback \(most recent call last\):\s*`);
const EXCEPTION_GROUP_HEADER = py(String.raw `(?:\+\s+)?Exception Group Traceback \(most recent call last\):\s*`);
const TRACEBACK_WRAPPER_LINE = py(String.raw `(?i)^(?:g?make|ninja):`);
const TRACEBACK_CHAIN_LINE = py(String.raw `(?i)^(?:during handling of the above exception|the above exception was the direct cause)`);
const TRACEBACK_RUNNER_FOOTER_LINE = py(String.raw `(?i)^(?:process|command) exited with code \d+|^exit(?:ed)? code:? \d+`);
function isPythonExceptionLine(line) {
    const stripped = strip(line);
    if (!stripped || matchStart(TRACEBACK_WRAPPER_LINE, stripped))
        return false;
    const colon = stripped.indexOf(":");
    const typeName = colon === -1 ? stripped : stripped.slice(0, colon);
    return typeName.length > 0 && typeName.split(".").every(isIdentifier);
}
function isBoundaryLine(stripped) {
    return (matchStart(TRACEBACK_WRAPPER_LINE, stripped) ||
...
function tracebackException(lines) {
    const headers = [];
    lines.forEach((line, index) => {
        if (fullMatch(TRACEBACK_HEADER, line))
            headers.push(index);
    });
    if (headers.length === 0)
        return null;
    ...
    for (let position = headers.length - 1; position >= 0; position--) {
        ...
        if (terminal && isPythonExceptionLine(terminal)) {
            ...
            for (let offset = terminalIndex - 1; offset >= 0; offset--) {
                ...
                if (!stripped)
                    continue;
                if (stripped.startsWith('File "') || isBoundaryLine(stripped))
                    break;
                if (isSpace(line.slice(0, 1)))
                    continue;
                if (isPythonExceptionLine(line))
                    exceptionStart = offset;
            }
 ...
 */
export function normalizeError(content) {
    if (!content)
        return "";
 ...
 */
export function fingerprint(toolName, content) {
