...
function runOutsideHostWorkScope(run) {
    let wrapped = run;
    for (const slot of HOST_WORK_SCOPE_SLOTS) {
        const storage = globalThis[slot];
...
export default function register(api) {
    ...
    if (hookPolicy?.allowConversationAccess !== true) {
        warn(`needs plugins.entries.${api.id}.hooks.allowConversationAccess = true in openclaw.json; ` +
    ...
    const remember = (sessionId, block) => {
        ...
        if (!entries.some((entry) => entry.block.hash === block.hash))
            entries.push({ block, shownAtMs: Date.now() });
    ...
    const llm = hostComplete()
        ? {
            async complete(systemPrompt, userMessage, timeoutMs) {
                ...
                const call = Promise.resolve().then(() => complete({
