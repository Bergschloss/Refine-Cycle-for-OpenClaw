...
export interface Source {
  /** Where the text came from, for the record: a file path or `lesson:<id>`. */
  name: string;
  text: string;
}
...
const STOPWORDS = new Set(
  (
    "the and for with that this from are was were not but you your have has had can " +
    ...
    "path url n x t true false null none undefined"
  ).split(" "),
);
...
export function terms(text: string): string[] {
  ...
  for (const match of text.toLowerCase().matchAll(/[\p{L}\p{N}_]+/gu)) {
    const word = match[0];
    if (word.length < 3 || STOPWORDS.has(word) || /^\p{N}+$/u.test(word)) continue;
    out.push(word);
  }
  return out;
}
...
function paragraphs(text: string): Paragraph[] {
  ...
  const flush = () => {
    ...
    if (joined) out.push({ line: start + 1, text: joined, terms: new Set(terms(joined)) });
    buffer = [];
  };
 ...
 */
export function findCoveringRule(tool: string, shape: string, sources: Source[]): Covering | null {
  ...
  for (const source of sources) {
    for (const paragraph of paragraphs(source.text)) {
      const namesTool =
        paragraph.text.toLowerCase().includes(toolLower) ||
      ...
      if (shared >= needed) return { source: source.name, line: paragraph.line, excerpt: excerpt(paragraph.text) };
    }
  }
  return null;
}
...
export function findRestatement(lesson: string, sources: Source[], threshold = 0.7): Covering | null {
  const lessonTerms = [...new Set(terms(lesson))];
  if (lessonTerms.length < 4) return null;
  for (const source of sources) {
    for (const paragraph of paragraphs(source.text)) {
