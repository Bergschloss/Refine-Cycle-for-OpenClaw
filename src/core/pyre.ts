 ...
 */
export function py(source: string, flags = ""): RegExp {
  let src = source;
  let extra = "";
  if (src.startsWith("(?i)")) {
    src = src.slice(4);
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (c === "\\") {
      ...
      i++;
      switch (n) {
        case "w": out += inClass ? WORD : `[${WORD}]`; break;
        case "W":
        ...
        case "D":
          ...
          out += "\\P{Nd}"; break;
        case "s": out += inClass ? SPACE : `[${SPACE}]`; break;
        case "S":
          if (inClass) throw new Error("\\S inside a class is not supported");
          out += `[^${SPACE}]`; break;
        case "b": out += inClass ? "\\x08" : BOUNDARY; break;
        case "B": out += NON_BOUNDARY; break;
        default: out += "\\" + n;
      }
      continue;
    }
...
/** A regex match with group spans, as Python's `re.Match` offers them. */
export interface Match {
  /** Group texts; index 0 is the whole match. Unmatched groups are `undefined`. */
  readonly groups: ReadonlyArray<string | undefined>;
  /** Start of the whole match in the original string. */
  readonly start: number;
  /** `[start, end]` of a group in the original string, or undefined if it did not match. */
...
export function subText(pattern: RegExp, text: string, replacement: string): string {
