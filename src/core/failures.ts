...
function commandKey(command: string[]): string {
  const words = [...command];
  while (words.length > 1 && (WRAPPERS.has(words[0].toLowerCase()) || /^[A-Za-z_][A-Za-z0-9_]*=/.test(words[0]))) words.shift();
...
function leadingCommand(args: Record<string, unknown>): string | null {
  for (const key of ["command", "cmd", "script"]) {
    ...
    else if (Array.isArray(value) && value.every((word) => typeof word === "string")) segments = value.length > 0 ? [value as string[]] : [];
    else continue;
