...
function commandKey(command: string[], depth: number, argv = false): string {
  ...
  for (let i = 1; i < words.length; i++) {
    ...
    if (!argv && /^(?:\d*|&)>>?$/.test(word)) {
      ...
      if (target !== "/dev/null") found.push(">", target); // where the command writes
      continue;
    }
    if (!argv && /^(?:\d*|&)>/.test(word)) {
      if (!/^(?:\d*|&)>>?(?:&\d*|\/dev\/null)$/.test(word)) found.push(word);
      continue;
    }
    if (word.startsWith("-")) {
      // A switch alone does not count; a value written into it does.
      if (/^--?[^=]+=/.test(word) || /^-[^-].+/.test(word)) found.push(word);
      continue;
    }
    found.push(word);
  }
  ...
  else if (stdin.length > 0) found.push("<<", code(stdin.join("\n")));
  return found.join(" ");
}
