...
function findSkillFiles(dir: string, depth: number, out: string[]): void {
...
export function readSources(workspaceDir: string | undefined, instructionFiles: string[], skillDirs: string[]): Source[] {
  const sources: Source[] = [];
  if (workspaceDir) {
    for (const name of instructionFiles) {
      const file = path.join(workspaceDir, name);
      const text = readText(file);
      if (text) sources.push({ name: file, text });
    }
  }
